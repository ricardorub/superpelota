...
public String misReservas(Model model, Principal principal, RedirectAttributes redirectAttributes) {
    ...
    try {
        ...
        // 2. Pasadas: SOLO confirmadas (CONFIRMADA) que ya ocurrieron
        List<Reserva> reservasPasadas = reservaService.obtenerReservasPorUsuarioYEstado(usuario, "CONFIRMADA");
        if (reservasPasadas == null) {
            reservasPasadas = new ArrayList<>();
        }
        // Filtrar solo las que ya pasaron (fecha de fin anterior a ahora)
        reservasPasadas = reservasPasadas.stream()
                .filter(r -> r.getFechaHoraFin().isBefore(LocalDateTime.now()))
